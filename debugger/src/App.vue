<template>
  <div class="h-full flex">
    <div class="p-4 bg-base-200 flex flex-col gap-2 w-80">
      ...
      <BaseButton @click="() => (started ? onStop() : onStart())">{{
        started ? "Stop" : "Start"
      }}</BaseButton>
    </div>
    <div class="flex flex-col flex-1">
      <div class="flex items-center gap-4 p-3 bg-base-200">
        ...
        <input
          @input="(ev: Event) => source?.goTo(+(ev.target as HTMLInputElement).value)"
          type="range"
          min="0"
          :max="frame_count"
          :value="frame_number"
      ...
      <base-tabs
        ...
        v-model="active_tab"
      />
      <div class="w-full relative flex-1 flex-col flex overflow-auto">
        <div
          class="absolute top-0 right-0 border-4 font-mono text-primary-content border-4"
          :style="{
            color: '#ffff',
          }"
        >
      ...
      <div
        class="text-xs overflow-y-auto bg-base-200 font-mono h-40 flex border-t-2 gap-2"
      >
        <div class="w-1/4 p-4">
          {{ config }}
        </div>
...
watch(
  config,
  () => {
    if (config.value) source.value?.sendConfig(config.value);
  },
  { deep: true }
);
